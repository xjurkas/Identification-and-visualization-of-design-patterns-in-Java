...
P-MARt v1.2 [2] definuje **11 Observer inštancií** v 6 z 9 projektov — druhý najpočetnejší vzor v benchmarku:

| Projekt | Počet GT | Kanonické Subject triedy (Observer) |
|---|---|---|
| jhotdraw | 2 | `AbstractFigure` (FigureChangeListener), `StandardDrawingView` (DrawingChangeListener/Painter) |
...
**Maximálna dosiahnuteľná Recall: 9/11 = 0.818** (Lexi 2 inštancie sú nedetekovateľné — Lexi má 0 výsledkov vo všetkých pokusoch; Lexi projekt má len 23 tried a štruktúra listenerov je tam pravdepodobne cez EXTENDS vonkajšieho typu bez PARAMETER_TYPE match).
...
**Výsledky:** TP=9, FP=309, FN=2 → **Precision=0.028, Recall=0.818, F1=0.055**

**Záver:** Najvyšší recall (9/11) — zachytí všetkých kanonických TP vrátane `AbstractFigure`, `StandardDrawingView`, `TestResult`, `Report`, `ViewerModel`, `Dispatcher`, `DefaultDiagramModel`, `Layer`, `DiagramUI`. Ale masívne FP (318 subjectov) — `add(Figure)`, `addElement`, `addItem` matchujú na akúkoľvek triedu s kolekčnou add metódou. Netbeans 211 subjectov, jrefac 51.
...
**Výsledky:** TP=9, FP=154, FN=2 → **Precision=0.055, Recall=0.818, F1=0.103**
...
**Výsledky:** TP=9, FP=96, FN=2 → **Precision=0.086, Recall=0.818, F1=0.155**
...
**Výsledky:** TP=3, FP=34, FN=8 → **Precision=0.081, Recall=0.273, F1=0.125**
...
**Výsledky:** TP=8, FP=57, FN=3 → **Precision=0.123, Recall=0.727, F1=0.211**

**Záver:** Obnoví 5 z 6 TP, ktoré P4 stratil (signál C — `TestListener.addError`, `ViewerModelListener.viewerModelEvent`, `FigureChangeListener.figureChanged` majú update-like prefix). **Stráca však quickuml GT** — `DefaultDiagramModel`/`Layer` nie je v P5 výsledkoch (quickuml=0), pretože jeho notify signál `updateListeners` nesplní ani jeden z disjunktívnych filtrov P5. Tým pádom P5 má FN=3 (Lexi 2 + quickuml 1). Napriek tomu P5 **eliminuje 39 FP oproti P3** (57 vs 96), čo vedie k výrazne lepšej Precision (0.123 vs 0.086) a celkovému F1=0.211 — najvyšší zo všetkých pokusov. **P5 je teda skutočne najlepší pokus**, nie P3.
...
| TP | 9 | 9 | **9** | 3 | 9 |
| FP | 309 | 154 | **96** | 34 | 121 |
| FN | 2 | 2 | **2** | 8 | 2 |
| Precision | 0.028 | 0.055 | **0.086** | 0.081 | 0.069 |
| Recall | **0.818** | **0.818** | **0.818** | 0.273 | **0.818** |
| F1 | 0.055 | 0.103 | **0.155** | 0.125 | 0.128 |

**Poznámka o TP:** P1–P3 dosahujú TP=9 (Lexi 2 sú nedetekovateľné, strop 9/11=0.818). P5 dosahuje TP=8 — quickuml GT (`DefaultDiagramModel`/`Layer`) nie je v P5 výsledkoch (quickuml=0), pretože `updateListeners` nesplní P5 disjunktívne signály. FN=3 (Lexi 2 + quickuml 1).

---

## 5. Najlepší výsledok

**Najlepší výsledok dosiahol Pokus 5** — F1 = 0.211 (Precision = 0.123, Recall = 0.727). Pokus 3 dosahuje F1 = 0.155 (Precision = 0.086, Recall = 0.818).
...
4. **Disjunktívny behaviorálny signál (P5)** — kombinuje notify metódu, CALLS delegáciu a update-prefix na Observer; eliminuje ďalších 39 FP oproti P3 (57 vs 96) za cenu straty 1 TP (quickuml GT) a dosiahne F1=0.211

Pokus 3 zostáva najlepší z hľadiska **Recall** (0.818 vs 0.727) — ak je prioritou nájsť čo najviac GT inštancií, P3 je vhodnejší.
...
5. **Base FP je fundamentálne vysoký.** Observer dosahuje F1 = 0.155, čo je najnižšie zo všetkých doteraz testovaných vzorov okrem Factory Method. Príčina je štrukturálna bežnosť — **akákoľvek trieda s add-metódou a kolekčným field-om matchuje**. Aj najlepšie pravidlá filtrujú len štvrtinu kandidátov. Toto je fundamentálna limitácia štrukturálnej detekcie pre behaviorálne vzory (podobný záver ako pri Strategy, kde F1 = 0.152).
...
Nazar et al. [3] reportujú pre Observer F1 = 0.08–0.30 na P-MARt v závislosti od prístupu. Tsantalis et al. [4] dosahujú F1 = 0.12–0.35. Naše P3 (F1 = 0.155) je v strede tohto rozsahu — konzistentné s čisto Cypher-based prístupom bez similarity scoring. Recall = 0.818 je nad priemerom literatúry (typicky 0.6–0.8), ale precision je pod priemerom kvôli absencii ML features alebo intent-aware filtrov.
