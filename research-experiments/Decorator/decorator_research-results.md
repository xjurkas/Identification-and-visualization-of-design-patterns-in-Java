...
1. **Oprava v client.js** — bez neho by JUnit `TestDecorator` nebol detekovaný (recall by bol 0.500)
...
1. **Oprava v cliente je kľúčový prínos na úrovni grafu.** Riešenie problému jdtls resolúcie (CALLS hrany na konkrétny implementátor namiesto interface) na úrovni client.js — nie na úrovni Cypher — je čistejšie a robustnejšie riešenie. Bez tohto fixu by recall bol maximálne 50%.
