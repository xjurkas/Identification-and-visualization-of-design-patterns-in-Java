...
**Vytvor jednu Perspective pre všetky vzory.** Klikni knižka 📖 vľavo hore → **+ Create new perspective**:

- Name: `GoF Patterns`
- Database: `neo4j`

V tabe **Categories** pridaj nasledujúce kategórie (klik **+ Add category**, vyber label z dropdown):
...
**Obrázok #1 — Čistá kostra Singleton:**
- Spusti `Singleton Pattern` phrase
- Dismiss osamotené Class uzly (nechaj len Type)
- Dismiss neutrálne metódy (`getContents`, `loadImage`, atď.) ak ich Bloom pridá

**Obrázok #2 — Singleton v kontexte triedy:**
- Spusti phrase
- Pravý klik na Singleton uzol → **Expand → All**
- Zostane Clipboard so všetkými metódami, Notes iba na Singleton-relevantných uzloch
...
```cypher
// Označí Base Decorator triedy
MATCH (componentInterface:Interface)
WHERE EXISTS { MATCH (componentInterface)-[:IN_FILE]->() }
MATCH (baseDecoratorClass:Class)-[:IMPLEMENTS]->(componentInterface)
...
WITH componentInterface, baseDecoratorClass, baseDecoratorType, componentField
WHERE EXISTS {
  MATCH (baseDecoratorType)-[:DECLARES]->(ctor:Constructor)
        -[:HAS_PARAMETER]->(param)-[:PARAMETER_TYPE]->(paramType:Type)
  WHERE paramType.fqn = componentInterface.fqn
}

// ≥2 delegujúce metódy + ≥50% interface metód
WITH componentInterface, baseDecoratorClass, baseDecoratorType, componentField,
     [(componentInterface)-[:DECLARES]->(im:Method) | im] AS interfaceMethods
WITH componentInterface, baseDecoratorClass, baseDecoratorType, componentField,
     size(interfaceMethods) AS interfaceCount
WHERE interfaceCount >= 1
WITH componentInterface, baseDecoratorClass, baseDecoratorType, interfaceCount,
     [(baseDecoratorType)-[:DECLARES]->(dm:Method)-[:CALLS]->(cm:Method)
      WHERE cm.containerFqn = componentInterface.fqn | dm] AS delegatingMethods
WITH componentInterface, baseDecoratorClass, baseDecoratorType, 
     interfaceCount, size(delegatingMethods) AS delegCount
WHERE delegCount >= 2
  AND (toFloat(delegCount) / toFloat(interfaceCount)) >= 0.5

// Concrete decorator + concrete component existujú
WHERE EXISTS {
  MATCH (cd:Class)-[:EXTENDS]->(baseDecoratorClass)
}
AND EXISTS {
...
RETURN baseDecoratorClass.fqn AS decorator, componentInterface.fqn AS component
ORDER BY decorator;
```
...
MATCH (d:Decorator) WHERE d:Type
MATCH (c:Component)
MATCH (d)-[:DECLARES]->(ctor:Constructor)
...
// 6. Concrete Components (iné implementácie Component, nie dekorátory)
...
Drag uzly do prehľadných pozícií. Force-based layout v Bloome funguje slušne, ale pre BP potrebuješ symetriu — pomôž si ručne.
...
## C2. Argumenty pre obhajobu BP

### C2.1 Štruktúrny vs. doménový aspekt triedy

> "Pattern detekcia presne **izoluje uzly tvoriace vzor** od uzlov tvoriacich doménovú funkčnosť. Notes sú prítomné iba na pattern-relevantných uzloch, čím vizualizujeme, že vzor je **ortogonálny** k zvyšku triedy. Detekčný algoritmus teda neidentifikuje 'celé triedy', ale presne tie metódy, fieldy a vzťahy, ktoré tvoria implementáciu vzoru."

### C2.2 Provenance referencie (kľúčový argument)

> "Každá Note anotácia obsahuje **provenance referenciu** — presný súbor a riadok zdrojového kódu, kde je daný štrukturálny prvok deklarovaný. Tým spájame **abstraktnú reprezentáciu vzoru** (graf) s **konkrétnou lokalitou v kóde**, čo umožňuje programátorovi okamžite navigovať z vizualizácie na implementáciu. Údaje o pozícii pochádzajú z LSP odpovedí JDTLS (`Range.start`), ktoré sú resolvované na úrovni AST a sú teda 100% presné — nezávislé od formátovania zdrojového kódu."

V detail paneli Note v Bloome je viditeľná aj plná `sourceUri` a `editorUri` (vscode:// link), takže klik na Note priamo poskytuje navigačný link.

### C2.3 Class/Type dualita

> "Singleton (a Decorator/Iterator/Visitor) vzory sú v grafovej reprezentácii zachytené **dualitou Class/Type uzlov**, kde Class predstavuje syntaktickú deklaráciu z parsovania zdrojového kódu a Type sémantický typ resolvený jazykovým serverom JDTLS. Hrany pattern-relevantnej štruktúry vychádzajú z Type uzla, lebo Type je to, čo systém typov v Jave skutočne 'vidí'. Class uzol je viac-menej wrapper reprezentujúci syntaktickú deklaráciu."

### C2.4 Conditional Notes (špecifické pre Singleton)

> "Pre privátny vs. verejný konštruktor používame **podmienenú anotáciu** — ak je konštruktor privátny, Note označuje rolu `Private Constructor` ako enforce-ujúci prvok vzoru. Ak je verejný, Note explicitne upozorňuje, že Singleton **nie je striktne enforce-ovaný**, čo je užitočná diagnostická informácia o stave kódu."

---

## C3. Riešenie problémov
